...
public class ConnectManage {
    ...
    private CopyOnWriteArrayList<NettyRpcClientHandler> connectedHandlers = new CopyOnWriteArrayList<>();
    private Map<InetSocketAddress, NettyRpcClientHandler> connectedServerNodes = new ConcurrentHashMap<>();
        ...
        if (allServerAddress != null) {
            if (allServerAddress.size() > 0) {  // Get available server node
                ...
                for (int i = 0; i < connectedHandlers.size(); ++i) {
                    NettyRpcClientHandler connectedServerHandler = connectedHandlers.get(i);
                    SocketAddress remotePeer = connectedServerHandler.getRemotePeer();
                    if (!newAllServerNodeSet.contains(remotePeer)) {
                        log.info("Remove invalid server node " + remotePeer);
                        NettyRpcClientHandler handler = connectedServerNodes.get(remotePeer);
                        if (handler != null) {
                            handler.close();
                        }
                ...
                for (final NettyRpcClientHandler connectedServerHandler : connectedHandlers) {
                    SocketAddress remotePeer = connectedServerHandler.getRemotePeer();
                    NettyRpcClientHandler handler = connectedServerNodes.get(remotePeer);
                    handler.close();
    ...
    public void reconnect(final NettyRpcClientHandler handler, final SocketAddress remotePeer) {
        if (handler != null) {
    ...
    private void addHandler(NettyRpcClientHandler handler) {
    ...
    public NettyRpcClientHandler chooseHandler() {
